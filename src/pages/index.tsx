...
class Page extends React.PureComponent<AllProps> {
  public static getInitialProps({ store }) {
    ...
    const reduxStore: Store<IApplicationState, AnyAction> = store;
    // reduxStore.dispatch(navbarActions.enableDarkTheme(true));
  }
    ...
    return (
      <MainLayout>
        <div className="container-lg clearfix p-2">
          ...
          <div className="col-12 col-md-4 float-left border p-4">column example</div>
          <div className="col-12 col-md-4 float-left border p-4">from Primer CSS</div>
          <div className="col-12 p-4">
            ...
            />
            <ul className="list-style-none image-thumb">
              {_.map(testAvatarData, (eachAvatar: IAvatarData) => {
                return (
                  <li
                    key={_.random(1000000, true).toString()}
                    className="d-inline-block col-12 col-md-2 p-1 border m-1"
                  >
                    <img className="width-full avatar border-blue" src={eachAvatar.src} alt={eachAvatar.alt} />
                  </li>
                );
              })}
            </ul>
